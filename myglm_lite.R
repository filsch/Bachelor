...
# glmLiteTestFunction <- function(trend,data, correlation_matrix){
#   tranformation_matrix = t(chol(correlation_matrix))
#   if (trend == 'quadratic'){
#     formula = z ~ x*y + I(x^2) + I(y^2)
#   }
#   else if (trend == 'linear'){
...
#   else {cat('Choose either simple, linear or quadratic trend','\n'); return()}
#   
#   myglm = glmLite(trend,data,correlation_matrix)
#   data$z = data$z %*%
#   exactglm = glm(formula, )
# }

posteriorDistribution <- function(size_predictionx, size_predictiony, correlation_function = 'exponential', glm_est, GRFsigma2 = 1){
  prediction_grid = expand.grid(x = seq(1,size_predictionx), y = seq(1,size_predictiony));
  prediction_grid$z = numeric(2500)
  ...
  #En eller annen feil her. Unøyaktig
